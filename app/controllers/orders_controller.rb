class OrdersController < ApplicationController
  ...
  def order_params
    params.require(:order).permit(:email, :phone_number, :credit_card_name, :credit_card_number, :credit_card_expiration_month, :credit_card_expiration_year, :credit_card_security_code, :use_billing_address, billing_address_attributes: [:first_name, :last_name, :address_1, :address_2, :city, :state, :zip], shipping_address_attributes: [:first_name, :last_name, :address_1, :address_2, :city, :state, :zip], event_purchase_option_ids: []).merge({event_id: params[:event_id]})
  end
end
